...
根据定理~\ref{thm:de-morgan-set}，我们可以推导出：任意集合之间的恒成立式，如果其
...
% 比如，我们用
% \[
%     f: \left\{ \text{Alice},\text{Bob},\text{Cate} \right\} \to \left\{ 
%     \text{dog},\text{ice cream},\text{flower} \right\} 
% \]
% 来表示人与物两个集合中的“喜爱”这一关系，并规定
% \begin{align*}
%     f(\text{Alice}) &= \text{flower},\\
%     f(\text{Bob}) &= \text{ice cream},\\
%     f(\text{Cate}) &= \text{flower}.\\
% \end{align*}
% 这样一来，映射$f$的值域是$\left\{ \text{flower},\text{ice cream} \right\} $，而
% 并不是一开始的陪域$\left\{ \text{dog},\text{ice cream},\text{flower} \right\}$。
...
\textbf{势}\index{势|see{基数}}。对于有限集$A$，它的基数是一个自然数，记作
